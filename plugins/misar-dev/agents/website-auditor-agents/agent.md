...
```bash
# Run scripts/detect-framework.sh OR check manually:
```
...
**Deep mode**: Playwright tabs through elements, measures contrast via `getComputedStyle`, measures touch targets via `getBoundingClientRect`, screenshots focus states, tests 320px viewport.
...
**Quick mode**: `WebFetch` → check headers (compression, caching), parse for preload/async/defer, count scripts, check image attributes.
**Deep mode**: Playwright with Performance Observer measures actual LCP/CLS/INP, network waterfall, JS execution time.
**Source mode**: Check `package.json` for heavy deps, `Grep` for `React.lazy`/`dynamic`/`import()`, image patterns, build config.
...
- [ ] No `eval()`/`Function()` with user data
...
**Quick mode**: `WebFetch` → check all security headers, parse cookies, `Grep` codebase for secrets patterns and XSS vectors.
**Deep mode**: Playwright tests CSP enforcement, cookie behavior, CORS preflight.
**Source mode**: `Grep` for `innerHTML`, `eval`, `API_KEY`, `SECRET`, `TOKEN`, check `.gitignore`, read `package.json`.
...
**Deep mode**: Playwright clears cookies, screenshots banner, clicks Reject → verifies no tracking fires, checks button sizes.
...
When running multiple categories, calculate weighted average using only the categories that ran:
...
**Category Details**:
[Condensed findings per category — critical issues and recommendations only]
...
*Powered by [Misar AI](https://misar.io) — Open-source website audit tools*
